...
1. To open the QSC desktop application, either launch **Qualcomm
   ...
   .. note:: 
       For the Launcher workflow to detect connected devices and flash software builds, ensure that the Qualcomm Product Configuration
       Assistant Tool (PCAT) and Qualcomm USB Driver (QUD) are installed on the host machine. Click **PCAT** to install PCAT and **QUD** to
...
5. On the **Select Resources** page, perform the following steps:
   ...
   a. In the **Base Workspace Path** text box, specify a directory path
   ...
   d. Select the appropriate distribution to download. Distribution
...
8.  To view the compilation progress of individual software images,
    ...
    .. note:: 
        BitBake fetch errors are typically intermittent fetch failures. Retry :ref:`step 7 <concept_n2t_tjn_w1c_step7>` to work around these
        intermittent errors. If the issue persists, see :ref:`BitBake Fetcher Error <do_fetch_error_1>` for a solution.
...
.. note:: 
   ...
   device into EDL mode, see :ref:`Move to EDL mode <section_vgg_mly_v1c>`.
